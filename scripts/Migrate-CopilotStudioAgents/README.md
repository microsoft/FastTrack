...
- **Every target environment referenced in your department -> environment mapping must already exist** before you run the migration script - it resolves target environments by name via `pac env list` and does not create them. Create/provision each target environment (and assign it the appropriate Dataverse database/licensing) ahead of time.
